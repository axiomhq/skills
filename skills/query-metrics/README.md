...
- **`edge_url`** - The metrics query API is served from the edge URL, not `api.axiom.co`. Set this to your Axiom edge endpoint. If omitted, scripts fall back to `url`.
...
| `axiom-api` | Low-level authenticated API calls (uses `url`, not `edge_url`) |
